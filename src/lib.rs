...
fn jenks_breaks_optimized(data: Vec<f64>, num_classes: usize) -> PyResult<Vec<usize>> {
    ...
    if num_classes == 0 {
        // If no classes are provided, return a Python ValueError.
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
            "Number of classes must be a positive integer.",
        ));
    }
    if num_classes > n_data {
        // If the number of classes exceeds data points, return an error.
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
    ...
    let mut cumulative_sum = vec![0.0; n_data]; // Cumulative sum of data.
    let mut cumulative_sum_squares = vec![0.0; n_data]; // Cumulative sum of squares of data.

    // Fill the cumulative sum and sum of squares.
    cumulative_sum[0] = data[0];
    cumulative_sum_squares[0] = data[0] * data[0];
    for i in 1..n_data {
        cumulative_sum[i] = cumulative_sum[i - 1] + data[i];
        cumulative_sum_squares[i] = cumulative_sum_squares[i - 1] + data[i] * data[i];
    }

    // Initialize tables for class limits and variance combinations.
    let mut lower_class_limits = vec![0; n_data * num_classes];
    let mut variance_combinations = vec![f64::INFINITY; n_data * num_classes];

    // Helper closure to calculate the 1D index for our 2D DP tables.
    let idx = |i, j| i * num_classes + j;

    // Initialize the first class (j = 1) for all data points.
    for i in 0..n_data {
        lower_class_limits[idx(i, 0)] = 0; // Start of the first class.
        let sum = cumulative_sum[i];
        let sum_squares = cumulative_sum_squares[i];
        let variance = sum_squares - (sum * sum) / (i as f64 + 1.0); // Calculate variance.
        variance_combinations[idx(i, 0)] = variance;
    }
    ...
    let mut k = n_data - 1; // Start backtracking from the last data point.
    for j in (1..num_classes).rev() {
        break_indices[j - 1] = lower_class_limits[idx(k, j)] - 1;
        k = break_indices[j - 1];
    }
